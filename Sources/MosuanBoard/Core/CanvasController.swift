...
final class CanvasController: ObservableObject {
    ...
    func undo() { dynamicTrianglePlayback.stop(); canvas?.stopDynamicAnglePlayback(); canvas?.endDynamicTrianglePlaybackHistory(); canvas?.endDynamicTriangleParameterEditHistory(); canvas?.undo(); refreshState() }
    func redo() { dynamicTrianglePlayback.stop(); canvas?.stopDynamicAnglePlayback(); canvas?.endDynamicTrianglePlaybackHistory(); canvas?.endDynamicTriangleParameterEditHistory(); canvas?.redo(); refreshState() }
    func deleteSelected() { dynamicTrianglePlayback.stop(); canvas?.stopDynamicAnglePlayback(); canvas?.endDynamicTrianglePlaybackHistory(); canvas?.endDynamicTriangleParameterEditHistory(); canvas?.deleteSelected(); refreshState() }
    ...
    func refreshState() {
        canUndo = canvas?.canUndo ?? false; canRedo = canvas?.canRedo ?? false; hasSelection = canvas?.hasSelection ?? false
        selectionCount = canvas?.selectionCount ?? 0; rotationDegrees = canvas?.selectedRotationDegrees ?? 0; selectionFrame = canvas?.selectionBoundsInView ?? .zero
        dynamicAngleDegrees = canvas?.selectedDynamicAngleDegrees.map(Double.init); dynamicAnglePlaying = canvas?.isSelectedDynamicAnglePlaying ?? false
        dynamicTriangleDegrees = canvas?.selectedDynamicIsoscelesTriangleDegrees.map(Double.init); dynamicTriangleLegLength = canvas?.selectedDynamicIsoscelesTriangleLegLength.map(Double.init)
    }
}