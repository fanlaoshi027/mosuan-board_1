...
final class InkMetalView: MTKView {
    ...
    func beginDynamicTrianglePlaybackHistory() { guard !dynamicTrianglePlaybackHistoryActive else { return }; dynamicTrianglePlaybackHistoryActive = true; renderer.beginHistoryTransaction() }
    func endDynamicTrianglePlaybackHistory() { guard dynamicTrianglePlaybackHistoryActive else { return }; dynamicTrianglePlaybackHistoryActive = false; renderer.endHistoryTransaction(); notifyState(); draw() }
    func beginDynamicTriangleParameterEditHistory() { guard !dynamicTriangleParameterHistoryActive else { return }; dynamicTriangleParameterHistoryActive = true; renderer.beginHistoryTransaction() }
    func endDynamicTriangleParameterEditHistory() { guard !dynamicTriangleParameterHistoryActive else { return }; dynamicTriangleParameterHistoryActive = false; renderer.endHistoryTransaction(); notifyState(); draw() }
    func setSelectedDynamicIsoscelesTriangleDegrees(_ d: CGFloat) {
        ...
        if renderer.setSelectedDynamicIsoscelesTriangleDegrees(d) { if ownsTransaction { renderer.endHistoryTransaction(); notifyState(); draw() } else { onSelectionChanged?(); draw() } } else if ownsTransaction { renderer.endHistoryTransaction() }
    }
    func setSelectedDynamicIsoscelesTriangleLegLength(_ length: CGFloat) {
        let ownsTransaction = !dynamicTrianglePlaybackHistoryActive && !dynamicTriangleParameterHistoryActive
        if ownsTransaction { renderer.beginHistoryTransaction() }
        if renderer.setSelectedDynamicIsoscelesTriangleLegLength(length) { if ownsTransaction { renderer.endHistoryTransaction(); notifyState(); draw() } else { onSelectionChanged?(); draw() } } else if ownsTransaction { renderer.endHistoryTransaction() }
    }
    ...
    private func copySelectionToPasteboard() { guard let data = renderer.makeSelectionClipboardData() else { return }; let pasteboard = NSPasteboard.general; pasteboard.clearContents(); pasteboard.setData(data, forType: Self.mosuanClipboardType) }
    private func cutSelectionToPasteboard() { guard renderer.hasSelection, let data = renderer.makeSelectionClipboardData() else { return }; let pasteboard = NSPasteboard.general; pasteboard.clearContents(); guard pasteboard.setData(data, forType: Self.mosuanClipboardType) else { return }; renderer.deleteSelected(); notifyState(); draw() }
    private func pasteSelectionFromPasteboard() { guard let data = NSPasteboard.general.data(forType: Self.mosuanClipboardType), renderer.pasteSelectionClipboardData(data) else { return }; notifyState(); draw() }

    override func mouseDown(with event: NSEvent) {
        window?.makeFirstResponder(self); let p = makePoint(from: event)
        ...
        if renderer.hasSelection && !temporarySelectHeld && !isSelectionTool {
            if let frame = renderer.selectionBoundsInView(), frame.contains(CGPoint(x: CGFloat(p.x), y: CGFloat(p.y))) { selectionDrag = true; lastPoint = p; renderer.beginHistoryTransaction(); return }
            renderer.clearSelection(); onSelectionChanged?(); draw(); return
        }
        if isEraserTool && !temporarySelectHeld { renderer.beginHistoryTransaction(); eraserPoints = [p]; lastEraserPoint = p; eraseAlongPath([p]); return }
        if isDynamicAngleTool && !selectionModeActive { renderer.beginHistoryTransaction(); dynamicAngleDragID = renderer.commitDynamicAngle(at: p); active = true; lastPoint = p; onSelectionChanged?(); draw(); return }
        if isDynamicIsoscelesTriangleTool && !selectionModeActive { renderer.beginHistoryTransaction(); dynamicIsoscelesTriangleDragID = renderer.commitDynamicIsoscelesTriangle(at: p); active = true; lastPoint = p; onSelectionChanged?(); draw(); return }
        ...
        if selectionModeActive {
            ...
            if renderer.hasSelection, let frame = renderer.selectionBoundsInView(), frame.contains(CGPoint(x: CGFloat(p.x), y: CGFloat(p.y))) { renderer.beginHistoryTransaction(); selectionDrag = true; lastPoint = p; return }
    ...
    override func mouseDragged(with event: NSEvent) {
        ...
        if isEraserTool && !temporarySelectHeld { eraserPoints.append(p); if let last = lastEraserPoint { eraseAlongPath([last, p]) } else { eraseAlongPath([p]) }; lastEraserPoint = p; return }
        ...
        let c = renderer.canvasPoint(from: p); let pressure = event.pressure > 0 ? Float(event.pressure) : (points.last?.pressure ?? 1)
    ...
    override func mouseUp(with event: NSEvent) {
        smartLineWorkItem?.cancel(); let p = makePoint(from: event)
        ...
        if isPolygonTool { return }
        if isEraserTool && !temporarySelectHeld { eraserPoints.append(p); if let last = lastEraserPoint, simd_distance(last, p) > 0.001 { eraseAlongPath([last, p]) }; lastEraserPoint = nil; eraserPoints.removeAll(keepingCapacity: true); renderer.endHistoryTransaction(); notifyState(); draw(); return }
        if selectionModeActive {
            if lassoActive { appendLassoPoint(p); let shouldSelect = lassoPoints.count >= 3 && lassoPathLength() >= 8; if shouldSelect { _ = renderer.selectLasso(in: lassoPoints, operation: lassoOperation) } else if lassoOperation == .replace { renderer.clearSelection() }; lassoActive = false; lassoPoints.removeAll(keepingCapacity: true); lassoOverlay.update(points: [], visible: false); renderer.endHistoryTransaction(); notifyState(); draw(); return }
            polygonVertexDrag = nil; lineEndpointDrag = nil; dynamicIsoscelesTriangleDragID = nil; rotationCenterDrag = false; rotationDrag = false; resizeHandle = nil; selectionDrag = false; renderer.endHistoryTransaction(); notifyState(); draw(); return
        }
        guard isUserInteractionEnabledForTool && active else { return }
        let c = renderer.canvasPoint(from: p); let pressure = event.pressure > 0 ? Float(event.pressure) : (points.last?.pressure ?? 1)
        points.append(InkPoint(x: c.x, y: c.y, pressure: pressure))
        if isOneStrokeTool {
            ...
            case .line(let start, let end): renderer.commitLine(from: SIMD2(Float(start.x), Float(start.y)), to: SIMD2(Float(end.x), Float(end.y)))
            case .polygon(let vertices): renderer.commitPolygon(points: vertices)
            case nil: renderer.commitStroke(points)
            }
        } else if isLineTool || (isSmartLineTool && smartLineDetected) {
            let line = linePreview(from: points); if line.count >= 2 { renderer.commitLine(from: SIMD2(line[0].x, line[0].y), to: SIMD2(line[1].x, line[1].y)) }
    ...
    override func keyDown(with event: NSEvent) {
        ...
        if event.modifierFlags.contains(.command), let key = event.charactersIgnoringModifiers?.lowercased() { switch key { case "c": if renderer.hasSelection { copySelectionToPasteboard() }; return; case "x": cutSelectionToPasteboard(); return; case "v": pasteSelectionFromPasteboard(); return; default: break } }
    ...
    override func keyUp(with event: NSEvent) { if event.keyCode == 56 || event.keyCode == 60 { temporarySelectHeld = false; return }; if event.keyCode == 49 { spaceHeld = false; panDrag = false; return }; super.keyUp(with: event) }
    override func tabletPoint(with event: NSEvent) { switch event.phase { case .began: mouseDown(with: event); case .changed: mouseDragged(with: event); case .ended: mouseUp(with: event); case .cancelled: smartLineWorkItem?.cancel(); active = false; points.removeAll(keepingCapacity: true); renderer.setStroke([]); eraserPoints.removeAll(keepingCapacity: true); lastEraserPoint = nil; if dynamicTrianglePlaybackHistoryActive { dynamicTrianglePlaybackHistoryActive = false; renderer.endHistoryTransaction() } else { renderer.endHistoryTransaction() }; dynamicTriangleParameterHistoryActive = false; polygonModel.cancel(); polygonVertexDrag = nil; lineEndpointDrag = nil; dynamicAngleDragID = nil; dynamicIsoscelesTriangleDragID = nil; lassoActive = false; lassoPoints.removeAll(keepingCapacity: true); lassoOverlay.update(points: [], visible: false); draw(); default: break } }

    private func handlePolygonClick(at viewPoint: SIMD2<Float>) { let canvas = renderer.canvasPoint(from: viewPoint); let point = CGPoint(x: CGFloat(canvas.x), y: CGFloat(canvas.y)); if !polygonModel.isConstructing { polygonModel.begin(at: point); renderer.beginHistoryTransaction(); renderer.setStroke([InkPoint(x: canvas.x, y: canvas.y, pressure: 1)]); draw(); return }; if polygonModel.closeIfNearFirst(at: point) { commitPolygon(polygonModel.previewVertices); polygonModel.cancel(); renderer.setStroke([]); renderer.endHistoryTransaction(); notifyState(); draw(); return }; if polygonModel.append(at: point) { let preview = polygonModel.previewVertices.map { InkPoint(x: Float($0.x), y: Float($0.y), pressure: 1) }; renderer.setStroke(preview); draw() } }
...
private final class LassoOverlayView: NSView {
    ...
    override func draw(_ dirtyRect: NSRect) { guard visible, points.count >= 2 else { return }; let path = NSBezierPath(); path.move(to: NSPoint(x: CGFloat(points[0].x), y: CGFloat(points[0].y))); for point in points.dropFirst() { path.line(to: NSPoint(x: CGFloat(point.x), y: CGFloat(point.y))) }; if points.count >= 3 { path.close() }; path.lineWidth = 1.2; let dash: [CGFloat] = [5, 4]; path.setLineDash(dash, count: dash.count, phase: 0); NSColor.controlAccentColor.withAlphaComponent(0.9).setStroke(); NSColor.controlAccentColor.withAlphaComponent(0.06).setFill(); path.stroke(); path.fill() }
}
